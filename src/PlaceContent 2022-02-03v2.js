...
const // Airtable Table/View/Field strings
    PROGRAM_LOG_DATE = "Program Log Date",  // Program Log table
    MEDIA_PLAYBACK_DEVICES = "Media Playback Devices", // Media Playback Devies table
    CONTENTTYPES = "Content Types",         // Content Types
    CONTENTITEMS = "Content Items",         // Content Items table
    PROGRAMLOG = "Program Log",             // Program Log table
    PLACEMENTGROUPS - "Placement Groups",   // Placement Groups table
    ONAIREXPORT = "OnAir Export";           // Standard view for scripts
...
let programLogDate = parametersRecord.getCellValue(PROGRAM_LOG_DATE);
let playbackDevices = await base.getTable('MEDIA_PLAYBACK_DEVICES').getView(ONAIREXPORT).selectRecordsAsync();
debugMsg('playbackDevices', playbackDevices);
let contentTypes = await base.getTable(CONTENTTYPES).getView(ONAIREXPORT).selectRecordsAsync();
debugMsg('contentTypes', contentTypes);
let contentItems = await base.getTable(CONTENTITEMS).getView(ONAIREXPORT).selectRecordsAsync();
debugMsg('contentItems', contentItems);
let logEntries = await base.getTable(PROGRAMLOG).getView(ONAIREXPORT).selectRecordsAsync();
debugMsg('logEntries', logEntries);
let contentGroups = await base.getTable(PLACEMENTGROUPS).getView(ONAIREXPORT).selectRecordsAsync();
...
for (let contentGroupsRecord of contentGroups.records) {
    ...
    contentItemPointers = contentGroupsRecord.getCellValue(CONTENTITEMS);
...
LogEntries: for (let logEntry of logEntries.records) {
    ...
    ContentGroups: for (let contentGroupsRecord of contentGroups.records) {
        ...
        if (programTemplateMatchString === null) {
            ...
            if (logProgramTemplateName.search(programTemplateMatchString) >= 0) {
                ...
                traceMsg('Failed to match on template name');
                continue ContentGroups;
            }
        }
	...
	*/
    if (placementCandidates.length > 0) {
        ...
        base.getTable(PROGRAMLOG).updateRecordAsync(logEntry.id, {
...
while (updatedLogRecords.length > 0) {
    await base.getTable(PROGRAMLOG).updateRecordsAsync(updatedLogRecords.slice(0, 50));
