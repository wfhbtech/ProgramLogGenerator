...
const scriptVersion = "22-08-08 v17";
...
//          msglvl = tracelvl; 
      ...
      debugMsg("At top of placement group loop with group=" + thisPlacementGroup);
        ...
        if (
          ...
          debugMsg("Effective/expiration date fail for: " + thisPlacementGroup);
        ...
        if (programTemplateMatchString === null) {
          ...
          if (logProgramTemplateName.search(programTemplateMatchString) >= 0) {
            ...
            debugMsg("Failed to match on Program name for " + thisPlacementGroup);
            continue PlacementGroups;
          }
        }
        ...
        if (!todOK) {
          debugMsg("TOD check didn't pass for " + thisPlacementGroup);
          continue PlacementGroups;
        }
        ...
        if (groupPlacementCount < placementMax) {
          ...
          debugMsg(matchingContentGroup.getCellValueAsString("Placement Group") + " passed all rules.");
        } else {
          debugMsg(
            ...
            if (contentPointer.id === contentRecord.id) {
              if (logEntryType.search(contentRecord.getCellValueAsString("Item Content Type")) >= 0) {
                ...
                if (matchingContentGroup.getCellValue("Placement Order") != placementPriority ) {
                  debugMsg("Placement priority too low for placement group", matchingContentGroup.getCellValueAsString("Placement Group"));
                  continue PlacementGroups;
                }
      ...
      if (placementCandidates.length > 0) {
        infoMsg("Found placement Candidates", placementCandidates);
      } else {
...
infoMsg("contentGroupLastPlacedArray", contentGroupLastPlacedArray);
