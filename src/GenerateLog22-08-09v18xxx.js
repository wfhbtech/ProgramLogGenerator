...
const scriptVersion = "22-08-09 v18";
...
function printLastPlacedArray (myContentGroupLastPlacedArray){
    const lpaGroupRecordIdx = 0,    // index of last placed array
      lpaGroupupdateIdx = 1,        // index of last updated array
      lpaMinimumPlacements = 2,     // index of minimum placements array
      lpaPlacements = 3,            // index of placements array
      lpaItemRecordsIdx = 4,        // index of item records array
      lpacontentItemUpdatesIdx = 5; // index of content item updates array
    
      infoMsg ("============ Last Placed Info =======================");
      for (let lastPlacedEntry of myContentGroupLastPlacedArray) {
        ...
        infoMsg ("Placement Group: " + thisGroup + " updated: " + convertHMS(thisLastPlaced) + " Minimum Placements: " + thisMinimumPlacements + " Placements: " + thisPlacements);
        infoMsg ("============ Content Items ==========================");
        for (var i = 0; i < thisItemRecords; ++i) {
            let thisItemUpdated = thisContentItemUpdates[i];
            let thisItemRecord = thisItemRecords[i];
            infoMsg ("   Update: " + convertHMS(thisItemUpdated) + " Item: " + thisItemRecord.getCellValueAsString("Item Name"));
            }
        infoMsg ("============ Last Placed Info =======================");
    }
}        
    ...
    LogEntries: for (let logEntry of logEntries.records) {
      ...
      PlacementGroups: for (let aPlacementGroup of placementGroups.records) {
        ...
        if (itemContentTypeMatch) {
            debugMsg("Item Content Type match for: "  + placementInfo + " - " + contentTypeItem.getCellValueAsString("Item Content Type"));
          }
