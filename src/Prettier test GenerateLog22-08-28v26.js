...
const scriptVersion = "22-08-28 v28";
...
const station = debugMsg('Generating program log for ' + parametersRecord.getCellValueAsString('Station'));

output.markdown("### Expanding Program Templates for today's broadcast schedule for station: " + parametersRecord.getCellValueAsString('Station'));
...
for (let logDayDetailsRecord of logDayDetails.records) {
    ...
    let scheduleProgramName = logDayDetailsRecord.getCellValueAsString('Program With Cue');
                        ...
                        if (programTemplateFromProgramTemplateDetails === programTemplateFromPrograms) {
                            ...
                            newLogRecords.push({
                                fields: {
                                    ...
                                    'Optional': programTemplateDetailsRecord.getCellValue('Optional'),
...
async function placeContent () {
    ...
    */
    ...
    const lpaGroupRecordIdx = 0,    // Placement Group Record
      ...
      lpaItemRecordsIdx = 4,        // Content records Array
      lpacontentItemUpdatesIdx = 5; // Content records time last placed array
    ...
    if (placementGroups.records.length < 1 ) {
        ...
        for (let aPlacementGroupRecord of placementGroups.records) {
            ...
            if (contentItemPointers !== null) {
              ...
              output.markdown("# Error: Placement group has no content: "+ aPlacementGroupRecord.getCellValueAsString("Placement Group"));  
            }
        if (contentItemRecords.length != 0) {
            lastPlacedEntry.push(aPlacementGroupRecord, Math.floor(Math.random() * -10), groupPlacementMinimum, 0, 
            contentItemRecords, contentItemUpdates);
    ...
    LogEntries: for (let logEntry of logEntries.records) {
        ...
        debugMsg ("logEntryStart: " + logEntryStart + "   TraceLogTime: " + traceLogTime);

        if (traceLogTime == logEntryStart) {
            msglvl = traceLevel; 
            debugMsg ("Enabling trace for: " +convertHMS(logEntryStart) + "  (" + traceLogTime + ")");
            } else {
      ...
      debugMsg("At top of log entry loop with entry: ", convertHMS(logEntryStart) + "(" + logEntryStart + ")");
      ...
      PlacementGroups: for (let aPlacementGroup of placementGroups.records) {
        ...
        traceMsg("Effective date", aPlacementGroup.getCellValue("Effective Date"));
        traceMsg("Expiration date",aPlacementGroup.getCellValue("Expiration Date"));
        if (
        ...
        for (let lastPlacedEntry of contentGroupLastPlacedArray) {
          ...
          if (lastPlacedEntry[lpaGroupRecordIdx] === matchingContentGroup) {
            ...
            traceMsg(matchingContentGroup.getCellValueAsString("Placement Group") + " groupLastPlaced: " + convertHMS(groupLastPlaced));
            break;
          }
          ...
          for (let contentRecord of contentItems.records) {
            if (contentPointer.id === contentRecord.id) {
              if (logEntryType.search(contentRecord.getCellValueAsString("Item Content Type")) >= 0) {
                ...
                debugMsg("placementCandidates", placementCandidates);
      ...
      if (placementCandidates.length > 0) {
        debugMsg("Found placement Candidates", placementCandidates);
            ...
            base.getTable("Program Log").updateRecordAsync(
                logEntry.id, {
...
//            printLastPlacedArray (contentGroupLastPlacedArray); 
      ...
      debugMsg("===== Select oldest placement group candidate for ", logInfoString);
      ...
      traceMsg("oldestGroupIdx", oldestGroupIdx);
      debugMsg("matchingContentGroup", matchingContentGroup);
      ...
      debugMsg("Content will be placed from " + matchingItem.getCellValueAsString("Item Name") + "  Placement Group: ",
        matchingContentGroup.getCellValueAsString("Placement Group"));
      ...
      debugMsg("Reached end of placement loop for ", +logEntry.getCellValue("Sequence Start"));
    } // end of log entry placement loop



    // Update the new program log table with content placements
    output.markdown("### Updating Program Log table with content placements");
    await waitBetweenUpdates();
...
function printLastPlacedArray (myContentGroupLastPlacedArray) {
    const lpaGroupRecordIdx = 0,    // Placement Group Record
      lpaGroupupdateIdx = 1,        // Time Last Placed
      lpaMinimumPlacements = 2,     // Minimum Required Placements
      lpaPlacements = 3,            // Actual Placements so far
      ...
      for (let lastPlacedEntry of myContentGroupLastPlacedArray) {
        let thisGroup = lastPlacedEntry[lpaGroupRecordIdx].getCellValueAsString("Placement Group");
        ...
        let thisContentItemUpdates = lastPlacedEntry[lpacontentItemUpdatesIdx];
        if (thisLastPlaced < 0) {
            output.markdown ("Placed: . . . . " + "  Group: " + thisGroup + " Min: " + thisMinimumPlacements + " Act: " + thisPlacements);
        } else {
            output.markdown ("Placed: " + convertHMS(thisLastPlaced) + "  Group: " + thisGroup + " Min: " + thisMinimumPlacements + " Act: " + thisPlacements);
        }
        for (var i = 0; i < thisItemRecords.length; ++i) {
            let thisItemUpdated = thisContentItemUpdates[i];
            let thisItemRecord = thisItemRecords[i];
            if (thisItemUpdated < 0) {
                output.markdown ("                    Item: . . . . " + " Name: " + thisItemRecord.getCellValueAsString("Item Name"));
            } else {
                output.markdown ("                    Item: " + convertHMS(thisItemUpdated) + " Name: " + thisItemRecord.getCellValueAsString("Item Name"));
            }
        }
    }
    output.markdown ("=====================================================");
...
async function waitBetweenUpdates() {
    ...
    if (updateInterval < intervalLimit) {
        await new Promise(resolve => setTimeout(resolve, intervalLimit - updateInterval));
    }
...
debugMsg('parametersRecord: ', parametersRecord);
output.markdown("Trace Placement Group: " + tracePlacementGroup + " Trace Content Item: " + traceContentItem + " Trace Number: " + traceLevel); 
await generateLog();
