    ...
    */
    ...
    if (placementGroups.records.length < 1) {
        ...
        for (let aPlacementGroupRecord of placementGroups.records) {
            ...
            contentItemPointers =
                aPlacementGroupRecord.getCellValue('Content Items');
    ...
    LogEntries: for (let logEntry of logEntries.records) {
        ...
        */
        ...
        PlacementGroups: for (let aPlacementGroup of placementGroups.records) {
            ...
            let placementMax =
                matchingContentGroup.getCellValue('Maximum Placements');
            ...
            traceMsg(
                'Minimum Placement Interval for group: ' +
                    matchingContentGroup.getCellValueAsString(
                        'Placement Group'
                    ),
            ...
            for (let lastPlacedEntry of contentGroupLastPlacedArray) {
                ...
                if (
                    lastPlacedEntry[lpaGroupRecordIdx] === matchingContentGroup
                ) {
            ...
            contentItemPointers =
                matchingContentGroup.getCellValue('Content Items');
            ...
            ItemTypeCheck: for (let contentPointer of contentItemPointers) {
                ...
                for (let contentRecord of contentItems.records) {
                    if (contentPointer.id === contentRecord.id) {
                        if (
                            logEntryType.search(
                                contentRecord.getCellValueAsString(
                                    'Item Content Type'
                                )
                            ) >= 0
                        ) {
                            ...
                            if (placementPriority === -1) {
                                placementPriority =
                                    matchingContentGroup.getCellValue(
                                        'Placement Order'
                                    );
                            }
            ...
            if (itemContentTypeMatch) {
                debugMsg(
                    'For' +
                        ...
                        contentTypeItem.getCellValueAsString(
                            'Item Content Type'
                        )
                );
            } else {
        ...
        */
        ...
        updatedLogRecords.push({
            ...
            fields: {
                ...
                'Placement Group':
                    matchingContentGroup.getCellValue('Placement Group'),
                'Playback Device':
                    matchingItem.getCellValueAsString('Playback Device'),
        ...
        lastContentDuration = matchingItem.getCellValue(
            'Content Duration Secs'
        );
        ...
        traceMsg(
            'Content placed for ' + logEntry.getCellValue('Sequence Start')
        );
        traceMsg(
