...
var traceContentItem =
  parametersRecord.getCellValueAsString('Trace Content Item')
...
*/
  ...
  output.markdown(
    "# Expanding Program Templates for today's broadcast schedule"
  )

  // Retrieve all the tables/records needed from Airtable to produce the log

  const station = infoMsg(
      ...
      if (scheduleProgramName === coreProgramName) {
        ...
        for (let aPgmTemplateFieldName of pgmTemplateFields) {
          ...
          let programTemplateFromPrograms =
            aCoreProgramRecord.getCellValueAsString(aPgmTemplateFieldName)
              ...
              if (
                ...
                newLogRecords.push({
                  fields: {
                    ...
                    'Program Log Date':
                      parametersRecord.getCellValue('Program Log Date'),
                    DOW: parametersRecord.getCellValue('Day of Week'),
                    'Program Name':
                      aCoreProgramRecord.getCellValue('Core Program Name'),
                    'Program Start': logDayDetailsRecord.getCellValue(
                    ...
                    'Break Name':
                      programTemplateDetailsRecord.getCellValue('Break Name'),
                    'Max Duration':
                      programTemplateDetailsRecord.getCellValue('Max Duration'),
                    Sequence:
                      programTemplateDetailsRecord.getCellValue('Sequence'),
                    'Break Start':
                      programTemplateDetailsRecord.getCellValue('Break Start'),
                    'Included Types': programTemplateDetailsRecord.getCellValue(
                    ...
                    Optional:
                      programTemplateDetailsRecord.getCellValue('Optional'),
...
async function placeContent() {
  ...
  if (placementGroups.records.length < 1) {
    ...
    for (let aPlacementGroupRecord of placementGroups.records) {
      ...
      if (contentItemPointers !== null) {
        ...
        output.markdown(
          '## Error: Placement group has no content: ' +
            aPlacementGroupRecord.getCellValueAsString('Placement Group')
        )
      }
  ...
  LogEntries: for (let logEntry of logEntries.records) {
    ...
    debugMsg(
      'logEntryStart: ' + logEntryStart + '   TraceLogTime: ' + traceLogTime
    )
    ...
    PlacementGroups: for (let aPlacementGroup of placementGroups.records) {
      ...
      if (placementFailReason[placementIndex].length > 0) {
        placementFailGroup[placementIndex] =
          aPlacementGroup.getCellValueAsString('Placement Group')
        debugMsg(
          'Skipping Placement Group: ' + placementFailGroup[placementIndex]
        )
      ...
      if (itemContentTypeMatch) {
        debugMsg(
          'For' +
            placementInfo +
            'Item Content Type match found' +
            contentTypeItem.getCellValueAsString('Item Content Type')
        )
      } else {
    ...
    if (placementCandidates.length > 0) {
      ...
      if (!optionalEntry) {
        errorMsg(
          'Was unable to place any content for Placeholder at ',
          logEntry.getCellValue('Log Entry Time Str')
        )
      }
      continue LogEntries
    ...
    contentGroupLastPlacedArray[oldestGroupIdx][lpaGroupupdateIdx] =
      logEntryStart
    ...
    updatedLogRecords.push({
      ...
      fields: {
        ...
        'Included Types':
          matchingItem.getCellValueAsString('Item Content Type'),
    ...
    if (groupPlacementCount < lastPlacedEntry[lpaMinimumPlacements]) {
      if (
        !placementFailGroup.includes(
          groupLastPlacedRecord.getCellValueAsString('Placement Group')
        )
      ) {
        output.markdown(
          groupLastPlacedRecord.getCellValueAsString('Placement Group') +
...
function printLastPlacedArray(myContentGroupLastPlacedArray) {
  ...
  for (let lastPlacedEntry of myContentGroupLastPlacedArray) {
    let thisGroup =
      lastPlacedEntry[lpaGroupRecordIdx].getCellValueAsString('Placement Group')
    ...
    for (var i = 0; i < thisItemRecords.length; ++i) {
      ...
      if (thisItemUpdated < 0) {
        output.markdown(
          '                    Item: . . . . ' +
            ' Name: ' +
            thisItemRecord.getCellValueAsString('Item Name')
        )
      } else {
        output.markdown(
          '                    Item: ' +
