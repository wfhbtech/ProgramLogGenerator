...
"use strict";
const errorlvl = 0, warnlvl = 1, infolvl = 2, debuglvl = 3, tracelvl = 4;
let msglvl = warnlvl;
function errorMsg(tag, msg) {if (msglvl >= errorlvl) msgMsg("ERROR: ", tag, msg);}
function warnMsg(tag, msg) {if (msglvl >= warnMsg) msgMsg("WARNING: ", tag, msg);}
function infoMsg(tag, msg) {if (msglvl >= infolvl) msgMsg("INFO: ", tag, msg);}
function traceMsg(tag, msg) {if (msglvl >= tracelvl) msgMsg("TRACE: ", tag, msg);}
function debugMsg(tag, msg) {if (msglvl >= debuglvl) msgMsg("DEBUG: ", tag, msg);}
...
========================================================================================================================
*/
output.markdown("# Ready to place content in Program Log");
infoMsg("Placing Content in Program Log");
let updatedLogRecords = [];
// Will hold updates to the Program Log table for  processing in batches (Airtable restriction)
...
let parametersRecord = await input.recordAsync( "Select Template to be used", base.getTable("Generate Log"));
...
const lpaGroupRecordIdx = 0, lpaGroupupdateIdx = 1, lpaGroupPriIdx = 2, lpaPlacements = 3, lpaItemRecordsIdx = 4, lpacontentItemUpdatesIdx = 5;
...
/*
     Main Loop: Attempt to place content in each Program Log Record 
========================================================================================================================
========================================================================================================================
*/
LogEntries: for (let logEntry of logEntries.records) {
  ...
  // Loop thru each Content Group looking for placement matchs
	ContentGroups: for (let contentGroupsRecord of contentGroups.records) {
		let incCount = 0, exCount = 0, includedMinimumTod = 0, includedMaximumTod = 0, excludedMinimumTod = 0, excludedMaximumTod = 0;
    ...
    if ( (programLogDate >= contentGroupsRecord.getCellValue("Effective Date") || contentGroupsRecord.getCellValue("Effective Date") === null) 
          &&
          (programLogDate <= contentGroupsRecord.getCellValue("Expiration Date") || contentGroupsRecord.getCellValue("Expiration Date") === null)) 
          {
            debugMsg("Placement Group effective/expiration date checks OK.");
		      } 
		else {
		...
		debugMsg("programMatchString: ", programMatchString);
		if (programMatchString === null) {
			...
			if (logProgramName.search(programMatchString) >= 0) {
				debugMsg( contentGroupsRecord.getCellValue("Placement Group") +	"  Match on Program Name:" + logProgramName + " programMatchString:" +
				programMatchString);
			}
			else {
				debugMsg("Failed to match on Program name");
				continue ContentGroups;
			}
		}
		...
		if (programTemplateMatchString === null) {
			...
			if (logProgramTemplateName.search(programTemplateMatchString) >= 0) {
				debugMsg( contentGroupsRecord.getCellValue("Placement Group") + "  Match on Program Template Name:" +
				logProgramTemplateName + " programTemplateMatchString:" + programTemplateMatchString);
			}
			else {
				debugMsg("Failed to match on Program name");
				continue ContentGroups;
		...
		debugMsg("Log entry DOW", logEntryDow);
		if (contentGroupDow === "") {
			contentGroupDow = "Mon/Tue/Wed/Thu/Fri/Sat/Sun";
		}
		debugMsg("Content group DOW Affinity", contentGroupDow);
		if (contentGroupDow.search(logEntryDow) >= 0) {
			...
			debugMsg( "NO DOW Affinity match ", contentGroupsRecord.getCellValueAsString("Placement Group"));
			continue ContentGroups;
		}
		...
		if (exCount === 0 && incCount === 0) {
			...
			if (excludedMaximumTod != -1 || excludedMinimumTod != -1) {
				...
				if ( logEntryStart >= excludedMaximumTod ||
				...
				if ( logEntryStart <= includedMaximumTod &&
		...
		if (!todOK) {
			debugMsg("TOD check didn't pass");
			continue ContentGroups;
		}
    debugMsg("TOD check AOK");
		...
		traceMsg( "Minimum Placement Interval for group: " +
		matchingContentGroup.getCellValueAsString("Placement Group"), minimumPlacementInterval);
		traceMsg( "Max placements for group: " +
		matchingContentGroup.getCellValueAsString("Placement Group"), placementMax);
    ...
    // Find the entry for the current content group in the contentGroupLastPlacedArray
		for (let lastPlacedEntry of contentGroupLastPlacedArray) {
			...
			if (lastPlacedEntry[lpaGroupRecordIdx] === matchingContentGroup) {
				...
				traceMsg( matchingContentGroup.getCellValueAsString("Placement Group") + " groupLastPlaced: " +
				groupLastPlaced);
				break;
			}
    ...
    // Use the list of content items in the current group record to search thru all
		...
		ItemTypeCheck: for (let contentPointer of contentItemPointers) {
			...
			for (let contentRecord of contentItems.records) {
				if (contentPointer.id === contentRecord.id) {
					if ( logEntryType.search( contentRecord.getCellValueAsString("Item Content Type")
					) >= 0
					) {
						let matchPair = [];
						...
						if ( matchingContentGroup.getCellValue("Placement Order") !=
						placementPriority
						) {
							continue ContentGroups;
						}
	...
	*/
	...
	contentGroupLastPlacedArray[oldestGroupIdx][
	lpaGroupupdateIdx
	] = logEntryStart;
	contentGroupLastPlacedArray[oldestGroupIdx][lpacontentItemUpdatesIdx][
	oldestItemIdx
	] = logEntryStart;
	...
	if ( logEntry.getCellValueAsString("Program Name") === lastProgram &&
	...
	debugMsg( "Content will be placed from " +
	matchingItem.getCellValueAsString("Item Name") +
	"  Content Group: ", matchingContentGroup.getCellValueAsString("Placement Group"));
	...
	updatedLogRecords.push({
		id: logEntry.id, fields: {
		Content: matchingItem.getCellValueAsString("Item Name"), "Content Placement Status": "Content Placed", "Included Types": matchingItem.getCellValueAsString("Item Content Type"), "Content Duration": matchingItem.getCellValue("Content Duration Secs"), "Content Group": matchingContentGroup.getCellValue("Placement Group"), "Playback Device": matchingItem.getCellValueAsString("Playback Device"), "Track/URL": matchingItem.getCellValueAsString("Track/URL"), Script: matchingItem.getCellValueAsString("Script"), "Content Start": adjustedStart, },
	});
...
output.markdown("# Content placed");