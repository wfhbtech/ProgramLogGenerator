...
const scriptVersion = "22-06-30 v13";
...
async function placeContent () {
    ...
    */
    ...
    let traceGroup = parametersRecord.getCellValue("Trace Placement Group");
    let traceItem = parametersRecord.getCellValue("Trace Content Item");
    let traceNumber = parametersRecord.getCellValue("Trace Number");
    ...
    if (contentGroups.length <1 ) {
        ...
        for (let contentGroupsRecord of contentGroups.records) {
            ...
            let groupPlacementMinimum = contentGroupsRecord.getCellValue("Minimum Placements");
        ...
        if (contentItemRecords.length != 0) {
            lastPlacedEntry.push(contentGroupsRecord, Math.floor(Math.random() * -10), groupPlacementMinimum, 0, contentItemRecords, contentItemUpdates);
    ...
    /*
    Placement rules loop: find all content items that meet all the plcement rules
    ...
    */
      ...
      ContentGroups: for (let contentGroupsRecord of contentGroups.records) {
        ...
        ItemTypeCheck: for (let contentPointer of contentItemPointers) {
          traceMsg("Content Item: ", contentPointer.name);
          traceMsg("Content Item id: ", contentPointer.id);
          for (let contentRecord of contentItems.records) {
            if (contentPointer.id === contentRecord.id) {
...
// item type check used to be here: iwashere
        // Use the list of content items in the current group record to search thru all
        // content item records that are in the current group.  For each content record
        // in the current group, check if its content type matches the current log entry's
        // required content type.
